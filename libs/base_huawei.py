...
class BaseHuaWei(BaseClient):
    ...
    async def add_address(self):
        await self.page.goto('https://devcloud.huaweicloud.com/bonususer/home/managebonus', {'waitUntil': 'load'})
        await asyncio.sleep(2)
        await self.page.click('li#Add')
        await asyncio.sleep(1)
        no_data = await self.page.querySelector('#add-table .no-data')
        if no_data:
            return
        await self.page.click('#add-adds')
        await asyncio.sleep(1)
        await self.page.type('#add-receive-name', '邹华')
        await self.page.type('#add-receive-phone', '18664845253')
        await self.page.type('#add-receive-area-info', '静安路6号55创意产业园3楼')
        await self.page.click('#add-info .devui-checkbox')
        await self.page.click('#ifDefault .devui-toggle')

        items = await self.page.querySelectorAll('#add-receive-area .devui-dropup')
        index = [13, 1, 5]
        for i, item in enumerate(items):
            await item.click()
            await asyncio.sleep(1)
            await self.page.click(f'.cdk-overlay-container .devui-dropdown-item:nth-child({index[i]})')
            await asyncio.sleep(1)

        await asyncio.sleep(1)
        await self.page.click('#adds-dialog .devui-btn-stress')
        await asyncio.sleep(2)
